...
import de.gematik.zeta.zetaguard.keycloak.plugins.smcb.opa.OpaGateEnforcer
import de.gematik.zeta.zetaguard.keycloak.plugins.smcb.opa.OpaGateInput
...
open class ZetaGuardTokenExchangeProvider(private val keystoreService: KeystoreService, private val opaConfig: OPAConfig) :
  ...
  /**
   * Adapted from [org.keycloak.protocol.oidc.tokenexchange.StandardTokenExchangeProvider.tokenExchange]
   *
   ...
   *
   * Unfortunately, the last check is for token type [org.keycloak.OAuth2Constants.ACCESS_TOKEN_TYPE], whereas we want token type
   * [org.keycloak.OAuth2Constants.JWT_TOKEN_TYPE].
   */
  ...
  private fun opaGate(context: ZetaGuardTokenExchangeContext): KeycloakValidation = either {
    ...
    val grantType = formParams.getFirst(org.keycloak.OAuth2Constants.GRANT_TYPE)
    ...
    when (val outcome = OpaGateEnforcer.enforce(httpClient, input, opaConfig, logger)) {
      is OpaGateEnforcer.Outcome.Skip -> context
      is OpaGateEnforcer.Outcome.Allow -> {
        context.accessTokenTtlSeconds = outcome.accessTokenTtl
        context.refreshTokenTtlSeconds = outcome.refreshTokenTtl
        context
      }
  ...
  private fun resolveAudiences(formParams: jakarta.ws.rs.core.MultivaluedMap<String, String>, context: ZetaGuardTokenExchangeContext): List<String>? {
